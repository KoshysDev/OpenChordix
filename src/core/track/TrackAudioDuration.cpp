...
namespace openchordix::track
{
    namespace
    {
        std::optional<int> readDurationSeconds(const std::filesystem::path &audioPath)
        {
    ...
    {
        const auto totalSeconds = readDurationSeconds(audioPath);
        if (!totalSeconds)
        {
            return std::nullopt;
        }
    ...
    {
        return readDurationSeconds(audioPath);
    }
}
